...
function pickAmountByLabel(
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!sameLineNumbers.length && i + 1 < lines.length) {
...
const templates: Template[] = [
  {
    ...
    parse: (raw) =>
      parseByLineRules(raw, "cmb_property", [
        ...
        { name: "招行保险持仓", label: "保险", assetClass: "insurance", excludeKeywords: ["收益"] }
      ])
  },
  {
    type: "cmb_wealth",
  ...
  },
  {
    ...
    parse: (raw) =>
      parseByLineRules(raw, "alipay_wealth", [
        { name: "支付宝余额", label: "余额", assetClass: "cash", excludeKeywords: ["收益"] },
        { name: "支付宝理财持仓", label: "理财", assetClass: "wealth_management", excludeKeywords: ["收益"] }
      ])
  },
  {
    type: "alipay_fund",
